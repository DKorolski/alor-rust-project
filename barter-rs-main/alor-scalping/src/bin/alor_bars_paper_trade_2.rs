...
const DEFAULT_TRADE_LOG: &str = "paper_trades_2.csv";
const DEFAULT_BARS_LOG: &str = "paper_bars_2.csv";
const DEFAULT_START_CASH: f64 = 30_000.0;
const DEFAULT_FROM_DATE: &str = "2025-12-01";
...
impl BarLogger {
    fn new(path: &str) -> anyhow::Result<Self> {
        ...
        let file = std::fs::OpenOptions::new()
            ...
            .append(true)
            .open(csv_path)
            .with_context(|| format!("failed to open bars log {path}"))?;
        let mut writer = csv::Writer::from_writer(file);
        if create_header {
            writer.write_record(["time", "open", "high", "low", "close"])?;
            writer.flush()?;
        }
        Ok(Self { writer })
    }

    fn log_bar(&mut self, bar: &Bar) -> anyhow::Result<()> {
        self.writer.write_record([
            bar.time.to_rfc3339(),
            format!("{:.4}", bar.open),
            format!("{:.4}", bar.high),
...
async fn main() -> anyhow::Result<()> {
    ...
    let bars_log = get_env_or_default("ALOR_BARS_LOG", DEFAULT_BARS_LOG);
    ...
    let mut bar_log = BarLogger::new(&bars_log)?;
            ...
            Message::Text(txt) => {
                if let Ok(payload) = serde_json::from_str::<Value>(&txt) {
                    ...
                    for bar in bars {
                        ...
                        strategy.on_bar(bar, trade_log, bar_log)?;
                    }
                }
            }
...
async fn fetch_history_batches(
    ...
    loop {
        ...
        let bars = read_history_batch(&mut ws_stream, first_msg, history_max_gap_min).await?;
        info!(
...
fn parse_date_env(key: &str) -> Option<i64> {
    std::env::var(key).ok().as_deref().and_then(parse_date)
}
