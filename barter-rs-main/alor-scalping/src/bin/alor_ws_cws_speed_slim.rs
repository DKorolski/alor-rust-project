...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let comment = format!("ws-speed-test:{tag}");
    ...
    let (create_resp, create_ack_ms) = create_limit_order(
        &mut cws_sink,
        &mut cws_stream,
        &portfolio,
        &symbol,
    ...
    if let Some((rec, _)) = &rec1 {
        ...
        println!("Не дождались события по ордеру после create (timeout).");
    }
    ...
    let (update_resp, update_ack_ms) = update_limit_order(
        &mut cws_sink,
        &mut cws_stream,
        &portfolio,
        &symbol,
    ...
    let (rec_new, wait_new_ms) = match wait_order_event(
        ...
        WaitOpts {
            ...
            predicate: Some(Box::new(move |r| {
                let sym_ok = symbol_of(r).map(|s| s == symbol_filter).unwrap_or(false);
                let port_ok = portfolio_of(r)
                    .map(|p| p == portfolio_filter)
                    .unwrap_or(false);
                sym_ok && port_ok && price_matches(price_from_ws(r), price_filter)
            })),
            timeout: Duration::from_secs(2),
            first_msg: None,
        },
    ...
    let (delete_resp, delete_ack_ms) = delete_limit_order(
        &mut cws_sink,
        &mut cws_stream,
        &portfolio,
        &exchange,
...
fn comment_of(rec: &Value) -> Option<String> {
    rec.get("comment")
        .or_else(|| rec.get("cmt"))
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
...
fn symbol_of(rec: &Value) -> Option<String> {
    rec.get("symbol")
        .or_else(|| rec.get("sym"))
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
...
fn portfolio_of(rec: &Value) -> Option<String> {
    rec.get("portfolio")
        .or_else(|| rec.get("p"))
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
...
fn price_from_ws(rec: &Value) -> Option<f64> {
    rec.get("price")
        .or_else(|| rec.get("px"))
        .and_then(|v| match v {
...
fn price_matches(price_val: Option<f64>, expected: f64) -> bool {
    match price_val {
        Some(p) => (p - expected).abs() < 1e-9,
        None => true,
    }
}
