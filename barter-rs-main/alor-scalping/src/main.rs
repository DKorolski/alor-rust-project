...
use crate::alor_market::alor_sih6_stream;
...
async fn main() -> anyhow::Result<()> {
    ...
    let cfg = StrategyConfig {
        ...
        exit_cluster_q: 0.05,
        max_cluster_depth_entry: 9,
        max_cluster_depth_exit: 10,
        entry_order_timeout_ms: 1000,
        exit_order_timeout_ms: 1500,
        ...
        exit_bid_thr: Some(4.0),
        exit_ask_thr: Some(4.0),
        order_placement_delay_ms: 0,
        csv_path: "trades_live_alor_sih6.csv".to_string(),
    };

    let mut engine = ScalpingEngine::new(cfg);
    let mut stream = alor_sih6_stream().await;
