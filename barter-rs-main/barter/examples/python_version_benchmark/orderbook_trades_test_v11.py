...
# ===== CONFIG V10_OFFLINE =====
OB_ZIP_PATH   = "2025-12-07_BTCUSDT_ob200.data.zip"
TRADES_PATH   = "BTCUSDT2025-12-07.csv.gz"

ROWS_TO_PROCESS: Optional[int] = 1_000_000
MAX_LEVELS     = 50
...
MIN_CLUSTER_DEPTH_ENTRY = 0

# Кластеры для выхода
EXIT_CLUSTER_Q          = 0.05
MAX_CLUSTER_DEPTH_EXIT  = 50
...
ENTRY_ORDER_TIMEOUT_MS   = 800  # Увеличили тайм-аут входа
EXIT_ORDER_TIMEOUT_MS    = 500

# Насколько кластер может отличаться от tp_plain (в тиках)
EXIT_CLUSTER_MAX_DIFF_TICKS = 495

# ---- параметр защитного выхода по движению против ----
ADVERSE_TICKS = 100  # сколько тиков против позиции терпим, прежде чем выходить maker-лимиткой
CLUSTER_DIFF = 1
...
class TradeResult:
    ...
    entry_price: float
    exit_price: float
    reason: str             # cluster_exit / time_stop_limit / eod_force / limit_exit / adverse_exit
    gross_ticks: float
    net_ticks: float
    gross_ret: float
    net_ret: float
    ...
    exit_fee: float
    total_fee: float
    exit_liquidity: str     # maker / taker (пока только maker)
    tp_plain: float
...
# ===== МОДИФИЦИРОВАННЫЙ BACKTEST V10_OFFLINE =====

def backtest_tick_v10_offline(ob: pd.DataFrame, tr: pd.DataFrame) -> pd.DataFrame:

    # пороги кластеров (по всему дню)
    ...
    entry_order_ts: Optional[int] = None

    # позиция
    entry_price: Optional[float] = None
    entry_ts: Optional[int] = None
    entry_time: Optional[pd.Timestamp] = None
    tp_plain: Optional[float] = None

    # выходная лимитка
    exit_order_price: Optional[float] = None
    exit_order_ts: Optional[int] = None
    exit_queue_ahead: float = 0.0
    exit_reason_target: Optional[str] = None  # cluster_exit / adverse_exit / None
    ...
    favorable_price: Optional[float] = None
    ...
    while i_ob < n_ob or i_tr < n_tr:
        ...
        if next_tr_ts is None or (next_ob_ts is not None and next_ob_ts <= next_tr_ts):
            ...
            if state == "in_position" and entry_price is not None:
                if side == +1:  # long — максимальный bbp
                    if favorable_price is None:
                        favorable_price = max(entry_price, bbp)
                    else:
                        favorable_price = max(favorable_price, bbp)
                elif side == -1:  # short — минимальный bap
                    if favorable_price is None:
                        favorable_price = min(entry_price, bap)
                    else:
                        favorable_price = min(favorable_price, bap)

            # 1) таймаут входной лимитки
            if state == "waiting_entry" and order_ts is not None:
                if ts - order_ts >= ENTRY_ORDER_TIMEOUT_MS:
                    state = "flat"
                    side = 0
                    order_price = None
                    order_ts = None
            ...
            # 2) таймаут выходной лимитки (кластер или защитный exit)
            ...
            # 3) кластерный выход (учитываем EXIT_CLUSTER_START_MS)
            if (
                state == "in_position"
                and entry_ts is not None
                and exit_order_price is None
                and tp_plain is not None
                and ts - entry_ts >= EXIT_CLUSTER_START_MS
            ):
                if side == +1:
                    is_big_ask = (
                        cav >= ask_exit_thr
                    ...
                    if is_big_ask:
                        ...
                        if abs(candidate - tp_plain) <= EXIT_CLUSTER_MAX_DIFF_TICKS * TICK_SIZE:
                            ...
                            exit_order_ts = ts
                            exit_queue_ahead = 0.0  # очередь не моделируем, MIT-поведение
            ...
            # 4) защитный выход при движении против на ADVERSE_TICKS тиков (maker по best bid/ask)
            if (
                state == "in_position"
                and entry_price is not None
                and exit_order_price is None   # кластер ещё не поставил лимитку
                and favorable_price is not None
            ):
                if side == +1:
                    adverse_ticks = (favorable_price - bbp) / TICK_SIZE
                    if adverse_ticks >= ADVERSE_TICKS:
                        # выходим как maker по лучшему ask
                        exit_order_price = bap
                        exit_order_ts = ts
                        exit_queue_ahead = max(bav, 0.0)  # вся текущая очередь перед нами
                        exit_reason_target = "adverse_exit"
                        n_exit_orders_placed += 1
                elif side == -1:
                    adverse_ticks = (bap - favorable_price) / TICK_SIZE
                    if adverse_ticks >= ADVERSE_TICKS:
                        # выходим как maker по лучшему bid
                        exit_order_price = bbp
                        exit_order_ts = ts
                        exit_queue_ahead = max(bbv, 0.0)
                        exit_reason_target = "adverse_exit"
                        n_exit_orders_placed += 1
            ...
            if state == "flat":
                is_bid_entry = (
                    ...
                    and cbd >= MIN_CLUSTER_DEPTH_ENTRY
                    #and cbv >= CLUSTER_DIFF * cav
                )
                is_ask_entry = (
                    cav >= ask_entry_thr
                    and cad is not None
                    and cad <= MAX_CLUSTER_DEPTH_ENTRY
                    and cad >= MIN_CLUSTER_DEPTH_ENTRY
                    #and cav >= CLUSTER_DIFF * cbv
                )

                if is_bid_entry and cbp is not None:
                    state = "waiting_entry"
                    side = +1
                    #order_price = cbp + 1 * TICK_SIZE  # как и раньше
                    candidate = cbp + TICK_SIZE  # "на 1 тик перед кластером"
                    # если в итоге это выше best bid – ограничиваемся best bid
                    if candidate > bbp + PRICE_EPS:
                        candidate = bbp
                    order_price = candidate

                    order_ts = ts
                    entry_order_ts = ts
                    # очередь: если стоим на best bid — вся текущая best bid очередь, иначе считаем себя первым
                    if abs(order_price - bbp) <= PRICE_EPS:
                        queue_ahead = max(bbv, 0.0)
                    else:
                        queue_ahead = 0.0
                    n_signals += 1
                    n_orders_placed += 1
                    ...
                    state = "waiting_entry"
                    side = -1
                    #order_price = cap - 1 * TICK_SIZE
                    candidate = cap - 1 * TICK_SIZE
                    if candidate < bap - PRICE_EPS:
                        candidate = bap
                    order_price = candidate
                    order_ts = ts
                    entry_order_ts = ts
                    if abs(order_price - bap) <= PRICE_EPS:
                        queue_ahead = max(bav, 0.0)
                    else:
                        queue_ahead = 0.0
                    n_signals += 1
                    n_orders_placed += 1
            ...
            # 1) исполнение входной лимитки (queue + проход цены)
            if state == "waiting_entry" and order_price is not None and side != 0:
                filled_entry = False

                if side == +1:
                    if price < order_price - PRICE_EPS:
                        pass  # пока не дошли до нашей цены
                    elif abs(price - order_price) <= PRICE_EPS:
                        queue_ahead -= size
                        if queue_ahead <= 0.0:
                            filled_entry = True
                    else:
                        # цена прошла выше нашей лимитки — считаем, что нас где-то по пути полностью исполнили
                        filled_entry = True
                elif side == -1:
                    if price > order_price + PRICE_EPS:
                        pass  # не дошли до нашей ask
                    elif abs(price - order_price) <= PRICE_EPS:
                        queue_ahead -= size
                        if queue_ahead <= 0.0:
                            filled_entry = True
                    else:
                        # цена ниже нашей лимитки — нас должны были исполнить
                        filled_entry = True
                ...
                if filled_entry:
                    ...
                    # инициализируем "лучшую" цену в сторону позиции
                    ...
                    # сбрасываем выходную лимитку, если она случайно была
                    exit_order_price = None
                    exit_order_ts = None
                    exit_queue_ahead = 0.0
                    exit_reason_target = None

            # 2) исполнение выходной лимитки (cluster_exit как MIT, adverse_exit c очередью)
            if state == "in_position" and entry_price is not None and exit_order_price is not None and side != 0:
                filled_exit = False

                if exit_reason_target == "cluster_exit":
                    # MIT: как только цена коснулась уровня или прошла его — считаем исполненным
                    if (side == +1 and price >= exit_order_price - PRICE_EPS) or (
                        side == -1 and price <= exit_order_price + PRICE_EPS
                    ):
                        filled_exit = True
                else:
                    # queue-модель для adverse_exit (maker по best bid/ask)
                    if side == +1:
                        if price < exit_order_price - PRICE_EPS:
                            pass
                        elif abs(price - exit_order_price) <= PRICE_EPS:
                            exit_queue_ahead -= size
                            if exit_queue_ahead <= 0.0:
                                filled_exit = True
                        else:
                            # цена ушла выше — где-то по пути нас должны были исполнить
                            filled_exit = True
                    elif side == -1:
                        if price > exit_order_price + PRICE_EPS:
                            pass
...
def summarize_trades(trades_df: pd.DataFrame):
    ...
    print("\n===== BACKTEST V10_OFFLINE (cluster + adverse + queue-fix + broker fee) =====")
    print(f"Сделок: {n}")
...
def save_trade_log(trades_df: pd.DataFrame, path: str = "scalp_trades_v10_offline_tradelog.csv"):
    if trades_df.empty:
...
def main():
    ...
    trades_df = backtest_tick_v10_offline(ob, tr)
    summarize_trades(trades_df)

    trades_df.to_csv("scalp_trades_v11_offline_full.csv", index=False)
    print("\nПолный DataFrame сделок сохранён в scalp_trades_v11_offline_full.csv")

    save_trade_log(trades_df, "scalp_trades_v11_offline_tradelog.csv")
