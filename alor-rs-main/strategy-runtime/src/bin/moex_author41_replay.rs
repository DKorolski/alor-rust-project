...
use strategy_runtime::strategies::moex_author41_42::{
    compare_author41_replay, load_model_bars, load_source_daily, load_source_trades,
    replay_ri_author41_dual_no_overlap_source, Author41ReplayComparison, ModelProfile,
    RegularSessionPolicy,
};
...
struct Report {
    profile_id: String,
    author41_variant: String,
    model_id: String,
    bars_loaded: usize,
    comparison: Author41ReplayComparison,
}
...
fn main() -> Result<()> {
    ...
    let replay = replay_ri_author41_dual_no_overlap_source(&bars, RegularSessionPolicy::moex_10m());
    let comparison = compare_author41_replay(&replay, &source_trades, &source_daily, cli.tolerance);
    let report = Report {
        profile_id: profile.profile_id.as_str().to_string(),
        author41_variant: profile.author41_variant.to_string(),
...
impl Cli {
    fn parse() -> Result<Self> {
        ...
        let mut model_id = "ri_author41_mr_primary".to_string();
