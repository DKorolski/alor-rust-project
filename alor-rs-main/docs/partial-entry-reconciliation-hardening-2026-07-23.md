...
It was deployed in a confirmed broker-flat window to:

- `Hybrid IMOEXF`, portfolio `7502MIW`;
- `Alor-USDRUBF`, portfolio `7502MIW`.

Only the two `strategy-runtime` services were recreated. Their gateway and
Redis services were not restarted. The prior image tag is retained in a
timestamped `.env` backup for rollback.
...
first newly received 10-minute bar. Both services must be `healthy` before the
