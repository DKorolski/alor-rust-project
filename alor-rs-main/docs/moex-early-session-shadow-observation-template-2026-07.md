...
For RI, reduce path events to one final state per `decision_key`, then count
each final-active key once. A repeated `shadow_path_active` row after restart
is not a second trade. Exclude keys whose latest path event is
`shadow_path_superseded`. `shadow_recorded` is an audit trace, not the final
no-overlap portfolio path.
